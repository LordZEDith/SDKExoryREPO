...
namespace ExorAIO.Utilities
{
    ...
    internal class Bools
    {
        ...
        public static bool HasSheenBuff()
            =>
                GameObjects.Player.HasBuff("sheen") || GameObjects.Player.HasBuff("LichBane") || GameObjects.Player.HasBuff("dianaarcready") ||
        ...
        {
            return target.MoveSpeed < 150 || target.HasBuff("rebirth") || target.HasBuff("chronorevive") || target.HasBuff("lissandrarself") ||
                target.HasBuff("teleport_target") || target.HasBuff("woogletswitchcap") || target.HasBuff("zhonyasringshield") ||
                target.HasBuff("aatroxpassivedeath") || IsValidStun(target as Obj_AI_Hero) || IsValidSnare(target as Obj_AI_Hero) ||
                target.HasBuffOfType(BuffType.Flee) || target.HasBuffOfType(BuffType.Taunt) || target.HasBuffOfType(BuffType.Charm) ||
        ...
        {
            return target.Buffs.Any(b => b.Type == BuffType.Snare && !Vars.InvalidSnareCasters.Contains(((Obj_AI_Hero)b.Caster).ChampionName));
        }
        ...
        {
            return target.Buffs.Any(b => b.Type == BuffType.Stun && !Vars.InvalidStunCasters.Contains(((Obj_AI_Hero)b.Caster).ChampionName));
        }
        ...
        public static bool ShouldCleanse(Obj_AI_Hero target)
            =>
                GameObjects.EnemyHeroes.Any(t => t.IsValidTarget(1500f)) && !Invulnerable.Check(GameObjects.Player, DamageType.True, false) &&
                    (target.HasBuffOfType(BuffType.Flee) || target.HasBuffOfType(BuffType.Charm) || target.HasBuffOfType(BuffType.Taunt) ||
                        target.HasBuffOfType(BuffType.Knockup) || target.HasBuffOfType(BuffType.Knockback) || target.HasBuffOfType(BuffType.Polymorph)
                        ||
        ...
        public static bool HasDeadlyMark()
            =>
                !Invulnerable.Check(GameObjects.Player, DamageType.True, false) && GameObjects.Player.HasBuff("zedrtargetmark") ||
                    GameObjects.Player.HasBuff("summonerexhaust") || GameObjects.Player.HasBuff("fizzmarinerdoombomb") ||
                    GameObjects.Player.HasBuff("vladimirhemoplague") || GameObjects.Player.HasBuff("mordekaiserchildrenofthegrave");
        ...
        public static bool IsPerfectRendTarget(Obj_AI_Base target)
        {
            ...
            {
                if (target.IsValidTarget(Vars.E.Range) && target.HasBuff("kalistaexpungemarker") && !Invulnerable.Check((Obj_AI_Hero)target))
                {
                    return true;
                }
            }
