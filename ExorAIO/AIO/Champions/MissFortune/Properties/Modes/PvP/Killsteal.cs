    ...
    internal partial class Logics
    {
        ...
        public static void Killsteal(EventArgs args)
        {
            ...
            if (Vars.Q.IsReady())
            {
                ...
                /// </summary>
                if (Vars.Menu["spells"]["q"]["killsteal"].GetValue<MenuBool>()
                                                         .Value)
                {
                ...
                /// </summary>
                if (Vars.Menu["spells"]["q"]["extended"]["exkillsteal"].GetValue<MenuBool>()
                                                                       .Value)
                {
                    /// <summary>
                                                                      ...
                                                                      t =>
                                                                          !Invulnerable.Check(t) && t.IsValidTarget(Vars.Q2.Range - 50f) &&
                                                                              ...
                                                                              ((Vars.PassiveTarget.IsValidTarget()
                                                                                  ...
                                                                                  Targets.Minions.All(
                                                                                                      m =>
                                                                                                          polygon.IsOutside((Vector2)m.ServerPosition))))
                           where target != null
                           where
                    ...
                    foreach (var target
                        in from target in GameObjects.EnemyHeroes.Where(t => t.IsValidTarget(Vars.Q.Range))
                           let polygon =
                               new Geometry.Sector((Vector2)target.ServerPosition,
                           ...
                           where
                               !polygon.IsOutside((Vector2)target2.ServerPosition) &&
                                   !polygon.IsOutside(
                                                      (Vector2)
                                                          Movement.GetPrediction(target2,
                                                              GameObjects.Player.Distance(target) / Vars.Q.Speed + Vars.Q.Delay)
                                                                  .UnitPosition)
                           select target)
                    {
