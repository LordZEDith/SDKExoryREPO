...
namespace ExorAIO.Champions.MissFortune
{
    ...
    internal partial class Logics
    {
        ...
        public static void Combo(EventArgs args)
        {
            ...
            ///     The W Combo Logic.
            /// </summary>
            if (Vars.W.IsReady() && !Bools.HasSheenBuff() && Vars.Menu["spells"]["w"]["combo"].GetValue<MenuBool>()
                                                                                              .Value)
            {
                if (GameObjects.EnemyHeroes.Any(t => t.IsValidTarget(Vars.Menu["spells"]["w"]["engager"].GetValue<MenuBool>()
                                                                                                        .Value
                    ? Vars.R.Range
                    : Vars.AARange)))
                {
            ...
            ///     The E Combo Logic.
            /// </summary>
            if (Vars.E.IsReady() && Targets.Target.IsValidTarget(Vars.E.Range) && !Invulnerable.Check(Targets.Target, DamageType.Magical, false) &&
                ...
                foreach (var minion 
                        ...
                        Targets.Minions.Where(
                                              m =>
                                                  m.IsValidTarget(Vars.Q.Range)
                                                      && (!Vars.Menu["spells"]["q"]["extended"]["excombokill"].GetValue<MenuBool>()
                                                                                                              .Value
                    ...
                    where
                        !polygon.IsOutside((Vector2)target.ServerPosition) &&
                            !polygon.IsOutside(
                                               (Vector2)
                                                   Movement.GetPrediction(target, GameObjects.Player.Distance(target) / Vars.Q.Speed + Vars.Q.Delay)
                                                           .UnitPosition)
                    select minion)
                {
