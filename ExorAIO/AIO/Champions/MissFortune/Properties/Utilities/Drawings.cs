            ...
            Drawing.OnDraw += delegate
                              {
                                  ...
                                  /// </summary>
                                  if (Vars.PassiveTarget.IsValidTarget() && Vars.Menu["drawings"]["p"].GetValue<MenuBool>()
                                                                                                      .Value)
                                  {
                                  ...
                                  /// </summary>
                                  if (Vars.Q.IsReady() && Vars.Menu["drawings"]["qc"].GetValue<MenuBool>()
                                                                                     .Value)
                                  {
                                      foreach (var obj in ObjectManager.Get<Obj_AI_Base>()
                                                                       .Where(m => m.IsValidTarget(Vars.Q.Range)))
                                      {
                                          var polygon = new Geometry.Sector((Vector2)obj.ServerPosition,
                                              (Vector2)obj.ServerPosition.Extend(GameObjects.Player.ServerPosition, -(Vars.Q2.Range - Vars.Q.Range)),
                                          ...
                                          if (target != null)
                                          {
                                              polygon.Draw(!polygon.IsOutside((Vector2)target.ServerPosition) &&
                                                  !polygon.IsOutside(
                                                                     (Vector2)
